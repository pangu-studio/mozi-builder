...
// ============================================================================
//
// Types and assembly logic live in mozi/changeplan; devplatform keeps thin
// aliases so existing consumers (CLI, HTTP handlers) see no API change.

// ChangePlanStatus indicates whether a change plan has been applied to code.
type ChangePlanStatus = changeplan.Status

const (
	ChangePlanPending ChangePlanStatus = changeplan.Pending // model changed, code not yet synced
	ChangePlanApplied ChangePlanStatus = changeplan.Applied // model changes already synced to code
	ChangePlanNoDiff  ChangePlanStatus = changeplan.NoDiff  // model has no version diff
)

// ChangePlanResult describes a model change as an AI Coding task instead of a
// template overwrite operation.
type ChangePlanResult = changeplan.Result

// ChangePlanTask is one actionable item in the AI Coding plan.
type ChangePlanTask = changeplan.Task
